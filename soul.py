...
def attack_command(message):
    ...
    try:
        ...
        bot.send_message(chat_id, "Enter the target IP, port, and duration.")
        bot.register_next_step_handler(message, process_attack_command)
    except Exception as e:
        logging.error(f"Error in attack command: {e}")
...
def process_attack_command(message):
    try:
        ...
        if len(args) != 3:
            bot.send_message(message.chat.id, "*Invalid command . Please use: /Attack target_ip target_port time*", parse_mode='Markdown')
            return
        ...
        bot.send_message(message.chat.id, f"*Chudai started \n\nIP: {target_ip}\nPORT : {target_port}\nTime: {duration}*", parse_mode='Markdown')
    except Exception as e:
...
def send_welcome(message):
    ...
    bot.send_message(message.chat.id, "*Choose an PLAN:*", reply_markup=markup, parse_mode='Markdown')
...
def handle_message(message):
    if message.text == "Instant Plan ":
        ...
        bot.reply_to(message, "*NOOB HO TUM*", parse_mode='Markdown')
    elif message.text == "Contact admin✔️":
        bot.reply_to(message, "*Contact @Choxdhxry*", parse_mode='Markdown')
    else:
        bot.reply_to(message, "*SHI DAL LODE*", parse_mode='Markdown')
